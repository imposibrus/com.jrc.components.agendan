...
var Job = module.exports = function Job(args) {
  ...
  attrs.type = attrs.type || e.attrs.type.once;
  this.attrs = attrs;
};
...
Job.prototype.toJSON=function(){ // create a persistable Mongo object -RR
    ...
    var dates = ['lastRunAt', 'lastFinishedAt', 'nextRunAt', 'failedAt', 'lockedAt','startRunsAt','noMoreAt'];
...
Job.prototype.computeNextRunAt = function() {
  ...
  function computeFromInterval() {
    ...
    try {
      ...
      if (isNaN(this.attrs.nextRunAt)) {
        this.attrs.nextRunAt = undefined;
        this.fail(e.msg.fail.invalidRepeat);
      } else {

        // startDate and endDate...
        _startsRun = (this.attrs.startRunsAt === undefined || this.attrs.startRunsAt === null) ? null : new Date(this.attrs.startRunsAt).valueOf();
        _endsRun = (this.attrs.noMoreAt === null) ? null : new Date(this.attrs.noMoreAt).valueOf();
        _nextRun = (this.attrs.nextRunAt === undefined || this.attrs.nextRunAt === null) ? null : new Date(this.attrs.nextRunAt).valueOf();

        if (!_endsRun && !_startsRun) { // no start and end date
          this.attrs.nextRunAt = _nextRun;
        } else if (!_endsRun && _startsRun) { // only start date
          this.attrs.nextRunAt = (_nextRun < _startsRun) ? _startsRun : _nextRun;
        } else if (_endsRun && !_startsRun) { // only end date
          this.attrs.nextRunAt = (_nextRun > _endsRun) ? null : _nextRun;
        } else if (_nextRun < _startsRun) { // both start and end date
          this.attrs.nextRunAt = _startsRun;
        } else if (_nextRun > _endsRun) { // both start and end date
  ...
  function computeFromRepeatAt() {
    ...
    if(Date.now() == date(repeatAt).valueOf()) {
      this.attrs.nextRunAt = undefined;
      this.fail(e.msg.fail.invalidFormat);
...
Job.prototype.fail = function(reason) {
  ...
  // log
  if (this.agenda && this.agenda._logs) this.log(this, reason);
  return this;
};
    ...
    setImmediate(function() {
      ...
      var jobCallback = function(err){
        if (err) {
          ...
          // log
          if (self.agenda._logs) self.log(self);
        }
      ...
      try {
        ...
        if(!definition) throw new Error(e.msg.fail.undefinedJob);
...
Job.prototype.log = function(job,err) {
  this.agenda.saveLog(job, err);
  return this;
};
