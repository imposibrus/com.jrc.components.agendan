...
var Agenda = module.exports = function(config) {
  ...
  this._processEvery = humanInterval(config.processEvery) || humanInterval(e.conf.default.processEvery);
  this._defaultConcurrency = config.defaultConcurrency || e.conf.default.concurrency;
  this._maxConcurrency = config.maxConcurrency || e.conf.default.maxConcurrency;
  this._definitions = {};
  this._runningJobs = [];
  this._jobQueue = [];
  this._defaultLockLifetime = config.defaultLockLifetime || e.conf.default.lockTime; // default lockLifetime
  if (config.logs)
    this.logs(config.logs.address, config.logs.collection);
  if (config.db)
...
Agenda.prototype.database = function(url, collection) {
  collection = collection || e.conf.default.dbName;
...
Agenda.prototype.logs = function(url, collection) {
  collection = collection || e.conf.default.dbName;
  if (!url.match(/^mongodb:\/\/.*/)) {
    url = 'mongodb://' + url;
  }

  this._logs = mongo.db(url, {w: 0}).collection(collection);

  ignoreErrors = function(){};
  this._logs.ensureIndex('job', ignoreErrors)
          .ensureIndex('time', ignoreErrors);
...
// Job Methods
Agenda.prototype.create = function(name, data, startDate, endDate) {
  var priority = this._definitions[name] ? this._definitions[name].priority : 0;
  var job = new Job({startRunsAt: startDate,noMoreAt: endDate,name: name, data: data, type: e.attrs.type.normal, priority: priority, agenda: this});
  return job;
};
...
Agenda.prototype.every = function(interval, names, data, startDate, endDate) {
  var self = this;

  if (typeof names === 'string') {
    return createJob(interval, names, data, startDate, endDate);
  } else if (Array.isArray(names)) {
    return createJobs(interval, names, data);
  }

  function createJob(interval, name, data, startDate, endDate) {
    var job;
    job = self.create(name, data, startDate, endDate);
    job.attrs.type = e.attrs.type.single;
...
Agenda.prototype.saveLog = function(job, err) {
...
Agenda.prototype.saveJob = function(job, cb) {
  ...
  if(id) {
  ...
  else if(props.type === e.attrs.type.single) {
    ...
    // Try an upsert.
    this._db.findAndModify({name: props.name, data: props.data,type: e.attrs.type.single}, {}, update, {upsert: true, new: true}, processDbResult);
  } else if(unique) {
